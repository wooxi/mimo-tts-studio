...
import { state, MODELS, PRESET_VOICES, scheduleSave, restoreConfig } from './store.js';
import { apiJson } from './api.js';
import { buildBody, validate, synthesize, synthesizeStream, fileToVoiceData } from './tts.js';
...
function renderModelCard() {
  ...
  if (info.presetVoices) {
    ...
    // 唱歌模式
    modelCard.appendChild(toggleRow('唱歌模式', 'singingToggle', state.singing, on => {
      state.singing = on;
      scheduleSave(syncBadge);
    }));
  ...
  if (state.model === 'mimo-v2.5-tts-voicedesign') {
    ...
    ta.rows = 3;
    ta.placeholder = 'Young female, warm and gentle voice, like a close friend whispering a bedtime story.';
    ta.value = state.voiceDesign;
    ta.addEventListener('input', () => { state.voiceDesign = ta.value; scheduleSave(syncBadge); });
    modelCard.append(label, ta);

    modelCard.appendChild(toggleRow('优化文本预览', 'optimizeToggle', state.optimize, on => {
      state.optimize = on;
      scheduleSave(syncBadge);
    }));
    const note = document.createElement('p');
...
initTags($('tagsPanel'), state, {
  onStyleTag(tag, on) {
    if (on) state.styleTags.add(tag); else state.styleTags.delete(tag);
...
// ---------- 输入绑定 ----------
$('textInput').addEventListener('input', () => { state.text = $('textInput').value; scheduleSave(syncBadge); });
$('styleInput').addEventListener('input', () => { state.style = $('styleInput').value; scheduleSave(syncBadge); });
  ...
  try {
    ...
    if (state.stream) {
      ...
      result = await synthesizeStream(body, bytes => {
        $('progressText').textContent = `接收中… ${(bytes / 1024).toFixed(1)} KB`;
      });
...
function loadParams(p) {
  ...
  if (p.voice) { state.voice = p.voice; }
  if (typeof p.style === 'string') { state.style = p.style; $('styleInput').value = p.style; }
  if (typeof p.text === 'string') { state.text = p.text; $('textInput').value = p.text; }
...
(async () => {
  const res = await apiJson('/api/history?limit=1').catch(() => ({ ok: true, status: 200 }));
  if (res.status === 401) {
