...
export function initOutput(onLoadParams) {
  ...
  audioEl.addEventListener('ended', () => { $('playBtn').textContent = '▶'; setPlayingItem(null); });
  ...
  $('historyList').addEventListener('click', e => {
    ...
    if (del) { deleteHistory(id, item); return; }
...
export async function showResult(blob, meta) {
  ...
  $('durationInfo').textContent = `WAV · ${formatDuration(meta.duration)} · ${(blob.size / 1024).toFixed(1)} KB`;
  $('timeDisplay').textContent = `0:00 / ${formatDuration(meta.duration)}`;
  audioEl.addEventListener('loadedmetadata', () => drawWaveform(objectUrl), { once: true });
...
function historyItemEl(item) {
  ...
  hiText.textContent = (item.text || '').replace(/\s+/g, ' ').slice(0, 60) || '(空)';
  ...
  hiVoice.textContent = item.voice || item.model.replace('mimo-v2.5-tts', 'MiMo');
  ...
  div.dataset.params = JSON.stringify({
    model: item.model, voice: item.voice, style: item.style, text: item.text,
  });
  return div;
}
...
async function drawWaveform(src) {
  try {
    ...
    ctx.fillStyle = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || 'oklch(0.78 0.14 75 / 0.6)';
    ctx.globalAlpha = 0.6;
    samples.forEach((s, i) => {
