...
export function initSettings() {
  ...
  streamToggle.addEventListener('click', () => {
    ...
    if (state.stream && state.format !== 'pcm16') {
      state.format = 'pcm16';
      document.querySelector('input[name="format"][value="pcm16"]').checked = true;
    }
    updateStreamNote();
    scheduleSave(syncBadge);
  });
...
export function syncBadge(status) {
  ...
  el.querySelector('.txt').textContent =
    status === 'ok' ? '已同步' : status === 'err' ? '同步失败' : status === 'off' ? '本地' : '同步中…';
}
