...
function newId() {
  let s = '';
  for (let i = 0; i < 12; i++) s += ID_ALPHABET[Math.floor(Math.random() * ID_ALPHABET.length)];
...
export async function pruneHistory(env) {
  ...
  for (const row of results) {
    await env.HISTORY_DB.prepare('DELETE FROM history WHERE id = ?').bind(row.id).run();
...
export async function handleHistory(request, env, cors, url, ctx) {
  ...
  if (request.method === 'POST' && path === '/api/history') {
    ...
    if (ctx?.waitUntil) ctx.waitUntil(prune); else await prune;
  ...
  if (request.method === 'DELETE' && delMatch) {
    const id = delMatch[1];
    const row = await env.HISTORY_DB.prepare('SELECT audio_key FROM history WHERE id = ?').bind(id).first();
    if (!row) return json({ error: 'not found' }, cors, 404);
    await env.HISTORY_DB.prepare('DELETE FROM history WHERE id = ?').bind(id).run();
    if (row.audio_key) await env.AUDIO_BUCKET.delete(row.audio_key);
...
export async function handleAudio(request, env, cors, url) {
  ...
  const row = await env.HISTORY_DB.prepare('SELECT audio_key FROM history WHERE id = ?').bind(match[1]).first();
  if (!row) return json({ error: 'not found' }, cors, 404);
